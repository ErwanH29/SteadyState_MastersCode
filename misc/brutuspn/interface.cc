...
bool Brutus::lightspeed_flagB = false;
...
mpreal epsilon = "1e-16"; // Bulirsch-Stoer tolerance
mpreal zzeta = "1"; // 1 AU, 1 MSun
...
// Set-Get zeta, n-body speed of light
int set_lightspeed_string(char* zt) {
    zzeta = zt;
    brutus->set_lightspeed(zzeta);
    return 0;
}
int get_lightspeed_string(char **zt) {
    result_strings[0] = zzeta.toString();
    *zt = (char*) result_strings[0].c_str();
    return 0;
}
int set_lightspeed(double zt) {
    zzeta = (mpreal)zt;
    brutus->set_lightspeed(zzeta);
    return 0;
}
int get_lightspeed(double *zt) {
    *zt = zzeta.toDouble();
    return 0;
}
int get_lightspeed_flag(bool *lf) {
    lightspeed_flag = brutus->get_lightspeed_flag();
    *lf = lightspeed_flag;
    return 0;
}
...
int calculate_word_length() {
    odata << t << ": changing L from " << numBits << endl;
    numBits = (int)( abs(4 * log10(epsilon)) + 33 );
    mpreal::set_default_prec(numBits);
    brutus->set_numBits(numBits);
    odata << numBits << " to " << brutus->get_numBits() << endl;
    return 0;
}
...
int get_energy(double *energy) {
    mpreal ener = brutus->get_energy();
    *energy = ener.toDouble();
    return 0;
}
