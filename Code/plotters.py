...
def spatial_plotter(init_dist):
    ...
    ax1.set_xlim(-2*init_dist.value_in(units.AU)**1.01, 2*init_dist.value_in(units.AU)**1.01)
    ax1.set_ylim(-2*init_dist.value_in(units.AU)**1.01, 2*init_dist.value_in(units.AU)**1.01)
    ...
    for i in range(len(IMBH_tracker)):
        ...
        if i != 0:
            ax2.plot(line_x[i][:]-line_x[1][:], line_y[i][:]-line_y[1][:], c = colors, lw = 1.4)
            ax2.scatter(line_x[i][0]-line_x[1][0], line_y[i][0]-line_y[1][0], 
                        alpha = 0.7, c = colors, edgecolors = 'black', s = 50)
            ax2.scatter(line_x[i][-1]-line_x[1][-1], line_y[i][-1]-line_y[1][-1], 
                        c = colors, edgecolors = 'black', s = 50)
        ax3.plot(time[0][5:], tdyn[i][5:], c = colors)
    ax1.scatter(0,0, color = 'black', s = 50)
