...
class IMBH_init(object):
    ...
    def IMBH_first(self, init_parti):
        ...
        gc_particles, rhmass = self.plummer_distr(init_parti)
        gc_particles.velocity = self.velocityList() * (1 | units.AU/units.yr)
        gc_particles.ejection = 0
        gc_particles.coll_events = 0

        gc_particles.radius = self.IMBH_radius(gc_particles.mass)
        gc_particles.collision_radius = self.coll_radius(gc_particles.radius)
        gc_particles.key_tracker = gc_particles.key
        gc_particles.mass = self.mass
        #gc_particles[0].velocity  = [0, 0, 0] | units.AU/units.yr
        gc_particles.mass = self.mass
        gc_particles[0].position = gc_particles.center_of_mass()
        
        """"""
        gc_particles.mass = self.mass

        convert = nbody_system.nbody_to_si(init_parti * self.mass, gc_code.gc_rad)
        gc_particles.scale_to_standard(convert_nbody=convert)
        
        SMBH_particle = Particles(1)
        SMBH_particle.key_tracker = SMBH_particle.key
        SMBH_particle.mass = SMBH_parti.mass
        SMBH_particle.radius = self.IMBH_radius(SMBH_particle.mass)
        SMBH_particle.collision_radius = self.coll_radius(SMBH_particle.radius)        
        SMBH_particle.position = [0, 0, 0] | units.AU
        SMBH_particle.velocity = [0, 0, 0] | units.AU/units.yr
        for part_ in gc_particles:
            part_.vy += (abs(constants.G*SMBH_parti.mass/gc_code.gc_dist)).sqrt()
        SMBH_particle.add_particle(gc_particles)
        SMBH_particle[1:].mass = self.mass
