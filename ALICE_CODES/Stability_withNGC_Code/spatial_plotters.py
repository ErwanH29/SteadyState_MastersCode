...
def energy_plotter():
    ...
    plot_ini = plotter_setup()
    count = file_counter()
    energy_tracker, col_len = file_opener('data/Hermite/energy/*')
    IMBH_energy_tracker, col_len = file_opener('data/Hermite/particle_energies/*')
...
def spatial_plotter(init_dist):
    ...
    count = file_counter()
    #ejec_parti, col_len = file_opener('data/Hermite/no_addition/chaotic_simulation/*')
    #com_tracker, col_len = file_opener('data/Hermite/center_of_mass/*')
    IMBH_tracker, col_len = file_opener('data/Hermite/particle_trajectory/*')
    """    Lag_tracker, col_len = file_opener('data/Hermite/lagrangians/*')
    com_x, com_y, com_z = file_manipulator(col_len, com_tracker)
    ...
    col_len = len(IMBH_tracker.iloc[0])
    ...
    for i in range(len(IMBH_tracker)):
        ...
        for j in range(col_len):
            ...
            tdynval = tIMBH_tracker.iloc[j][-1]
            if len(coords) == 1:
                pass
            else:
                line_x[i][j][0] = coords[0].value_in(units.pc)
